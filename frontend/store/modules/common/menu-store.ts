...
export class MenuStore extends VuexModule {
  ...
  addMenuClassname(payload: { className: string, currentClasses: string }) {
    let {className, currentClasses} = payload;
    this.menuType = !(currentClasses.indexOf(className) > -1)
  ...
  async mounted() {
    ...
    let logout: MenuItem = new MenuItem()
    logout.id = "logout"
    logout.icon = "house"
    logout.label = "menu.logout"
    logout.to = "/logout"

    let appointment: MenuItem = new MenuItem()
  ...
  async openSubMenu(menuItem: MenuItem) {
    ...
    if (!hasSubMenu) {
      ...
      if(this.viewingParentMenu === selectedParent) {
        this.changeMenuAfterOpening({menuType: this.CSS_MENU_SHOW_FIRST_LEVEL,
          menuState: this.MENU_STATE_SHOW_FIRST_LEVEL, viewingParentMenu: ""})
      } else {
        this.changeMenuAfterOpening({menuType: this.CSS_MENU_SHOW_SECOND_LEVEL,
          menuState: this.MENU_STATE_SHOW_SECOND_LEVEL, viewingParentMenu: selectedParent})
      }
    }
  }
  ...
  async handleWindowResize(event: { isTrusted: any; }) {
    ...
    let nextClasses = await this.getMenuClassesForResize(this.menuType);
    //this.changeSideMenuStatus();
  }
